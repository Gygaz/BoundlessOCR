...
class Worker(QThread):
    ...
    def process_pdf(self):
        ...
        reader = PdfReader(self.source[0])
        pages = len(reader.pages)

        if pages > 50:
            splitPath = os.path.join(self.dir_path, "splitPdfs", os.path.splitext(os.path.basename(self.source[0]))[0]).replace(" ", "_")
            os.makedirs(splitPath, exist_ok=True)
            self.status_message.emit("Splitting large pdfFileinto smaller subfiles")
            newsource = pdfSplitter.fragment_pdf(self.source[0], pages, splitPath)
            self.source = newsource

        self.status_message.emit(f"Document was split into {len(self.source)} parts")
        
        RealESRGAN= os.path.join(self.dir_path, "model", "RealESRGAN_x4plus_anime_6B.pth")
        upscaler= None

        for index, pdfFile in enumerate(self.source):
            ...
            for page, image_path in enumerate(processedList):                
                ...
                if os.path.exists(txt_path):
                    ...
                    continue
                self.status_message.emit(f"Scanning page {page+1} of {len(processedList)}")
                ocr = Scanner()
